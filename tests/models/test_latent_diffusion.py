"""
Tests for LatentDiffusionModel.

The pretrained VAE (stabilityai/sd-vae-ft-mse) is mocked so these tests run
without a network connection or GPU. Mark slow tests with @pytest.mark.slow
...
def _make_mock_vae(latent_b=2, img_h=32, img_w=32):
    ...
    # .to(device) must return the same mock so attribute access still works
    vae.to.return_value = vae
    # no real parameters — prevents requires_grad loop from crashing
    vae.parameters.return_value = iter([])

    # encoder: encode(x).latent_dist.{sample,mode}() → latent tensor
    mock_dist = MagicMock()
...
class TestEncodeDecode:
    ...
    def test_decode_divides_by_scaling_factor(self, model, mock_vae):
        ...
        # z / scaling_factor should be ≈ 1.0
