...
class DiffusionModel(BaseModel):
    ...
    def sample(
        ...
        **kwargs,
        ...
        if use_ddim:
            samples = self._sample_ddim(num_samples, device, image_size, batch_size, conditioning, T_test)
        else:
            if T_test != self.timesteps:
                log_once("DDPM sampling usually requires test_timesteps == train_timesteps. Switching to DDIM.")
                samples = self._sample_ddim(num_samples, device, image_size, batch_size, conditioning, T_test)
            else:
                samples = self._sample_ddpm(num_samples, device, image_size, batch_size, conditioning)
    ...
    @torch.no_grad()
    def _sample_ddim(self, num_samples, device, image_size, batch_size, conditioning, T_test):
        ...
        for idx in range(math.ceil(num_samples / batch_size)):
            ...
            for t_curr, t_next in time_pairs:
                ...
                if self.has_conditional_generation:
                    eps_all, _ = self.unet(torch.cat([x_t, x_t]), torch.cat([t_batch, t_batch]), conditioning=c_batch + u_batch)
    ...
    def _sample_ddpm(self, num_samples, device, image_size, batch_size, conditioning):
        ...
        for idx in range(math.ceil(num_samples / batch_size)):
            ...
            for t in reversed(range(self.timesteps)):
                ...
                mean = (1.0 / torch.sqrt(alpha_t)) * (x_t - (beta_t / torch.sqrt(1 - alpha_cum_t)) * eps_theta)
